...
namespace DoctorsDungeon.ComponentPattern.Particles
{
    ...
    {
        public static GameObject CreateParticleEmitter(string name, Vector2 pos, Interval speedX, Interval speedY, Interval direction, float particlesPerSecond, Interval maxAge, int maxAmount, double timeBeforeStop = -1, Interval rotation = null, Interval rotationVelocity = null)
        {
            GameObject go = new();
            go.Type = GameObjectTypes.Emitter;
            
            go.AddComponent<ParticleEmitter>(name, pos, speedX, speedY, direction, particlesPerSecond, maxAge, maxAmount, timeBeforeStop, rotation, rotationVelocity);
        ...
        {
            GameObject go = CreateParticleEmitter("Text Damage Taken", new Vector2(200, -200), new Interval(50, 100), new Interval(50, 100), new Interval(-MathHelper.Pi, 0), 0, new Interval(400, 600), 100, -1, new Interval(-0.5f, 0.5f), new Interval(-0.001f, 0.001f));
